...
void release(struct task_struct * p)
{
	...
	for (i=1 ; i<NR_TASKS ; i++)
		if (task[i]==p) {
			...
			free_page((long)p);//每个task_struct占用一页内存空间，该页前一部分为task_struct结构体，描述进程；剩余空间作为进程内核态栈空间，因此，内核态栈空间是不能扩展的
			schedule();
			return;
		}
 ...
 */
//指针只能是空或者指向具体struct task_struct
...
static inline int send_sig(long sig,struct task_struct * p,int priv)
{
	...
	if ((int) p->sigaction[sig-1].sa_handler == 1)/*#define SIG_IGN 1*/
		return 0;
...
int is_orphaned_pgrp(int pgrp)
{
	...
	for (p = &LAST_TASK ; p > &FIRST_TASK ; --p) {
		if (!(*p) ||
		    ...
		    ((*p)->p_pptr->pid == 1))
			continue;
                /*进程在进程pgrp组内，而其父进程不在组内，且他们属于同一会话*/
...
volatile void do_exit(long code)
{
	...
	if ((current->p_pptr->pgrp != current->pgrp) &&
	    ...
	    has_stopped_jobs(current->pgrp)) {
          kill_pg(current->pgrp,SIGHUP,1);/*杀死所有的进程组内进程*/
...
#endif
				return flag;
                  default:/*找到满足条件的进程，但是其处于运行太或者睡眠态*/
				flag=1;
				continue;
		}
	}
	if (flag) {/*找到满足条件的进程，但是其不处于立刻退出状态或者i僵死态*/
          if (options & WNOHANG)/*不挂起*/
			return 0;
		current->state=TASK_INTERRUPTIBLE;
		oldblocked = current->blocked;
		current->blocked &= ~(1<<(SIGCHLD-1));//允许接受SIG_CHLD
		schedule();
		current->blocked = oldblocked;
		if (current->signal & ~(current->blocked | (1<<(SIGCHLD-1))))/*接收到出SIGCHLD以外的其他未屏蔽信号*/
			return -ERESTARTSYS;
		else
			goto repeat;
	}
