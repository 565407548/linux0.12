...
3-5：group,rwx
6-8：user,rwx
9-12：01-set user id; 02-set group id;04-stick,对于目录受限删除标志
 */
...
/*
struct utimbuf 在 include/utime.h 定义
修改文件访问和修改时间
 */
...
int sys_open(const char * filename,int flag,int mode)
{
	...
	current->close_on_exec &= ~(1<<fd);//不应该为： current->close_on_exec |= (1<<fd) ??
	f=0+file_table;
